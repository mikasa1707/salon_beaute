import { Component, Input, OnDestroy, OnInit, ChangeDetectorRef, SimpleChanges, OnChanges } from '@angular/core';
...
export class ProduitUnites implements OnInit, OnDestroy, OnChanges {
    ...
    this.produitUniteService.findbyProduit(this.produit.id, this.page, this.limit, search).subscribe({
      next: res => {
        this.unites = res.data.map((u: any) => ({
          ...u,
          format: `${u.conversion} ${u.uniteMesure?.symbole ?? ''}`,
        }));
  ...
  async delete(id: number) {
    ...
    this.produitUniteService.remove(id).subscribe(() => {
      this.toast.error('Unité archivée');
      this.load(this.searchValue);
    });
  }
